    ...
    fun onUpdate() {
        ...
        firebaseDB.collection(DATA_USERS)
            ...
            .addOnFailureListener {
                progressLayout.visibility = View.GONE
                Toast.makeText(context, "Status Update failed. Please try again later.", Toast.LENGTH_LONG).show()
            }
    }

    private fun onUploadFailure() {
        Toast.makeText(context, "Image upload failed. Please try again later.", Toast.LENGTH_LONG)
            .show()
        progressLayout.visibility = View.GONE
    }
}
